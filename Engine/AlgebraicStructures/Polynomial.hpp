...
namespace math {
	...
	*/
	...
	public:
		...
		Polynomial(double coefficients_[]) {
			copyCoefficients(coefficients_);
		}
		...
		*/
		...
		const double& operator [](size_t at) const {
			return coefficients[at];
		}

		double& operator [](size_t at) {
			return coefficients[at];
		}
		...
		*/
		...
		Polynomial<DEGREE>& normalize() {
			...
			double norm_ = sqrt(scalar(*this, *this));
			for (size_t i = 0; i <= DEGREE; i++)
	...
	/**
		...
		Multiplication with scalar (vector space).
	...
	*/
	...
	inline const Polynomial<DEGREE_> operator +(
	...
	template <unsigned DEGREE_>
	inline const Polynomial<DEGREE_> operator +(
		const Polynomial<DEGREE_>& p, double alpha) {
	...
	template <unsigned DEGREE_>
	inline const Polynomial<DEGREE_> operator +(
		double alpha, const Polynomial<DEGREE_>& p) {
		return p + alpha;
	}
	...
	*/
	template <unsigned DEGREE_>
	const inline Polynomial<DEGREE_> operator *(
		double alpha, const Polynomial<DEGREE_>& p) {
	...
	*/
	template <unsigned DEGREE_>
	const inline Polynomial<2 * DEGREE_> operator *(
		const Polynomial<DEGREE_>& p, const Polynomial<DEGREE_>& q) {
	...
	*/
	template <unsigned DEGREE_>
	const inline Polynomial<DEGREE_> derivative(
	...
	*/
	template <unsigned DEGREE_>
	const inline double integrate(
		const Polynomial<DEGREE_>& p, double from, double to) {
	...
	*/
	template <unsigned DEGREE_>
	double norm(const Polynomial<DEGREE_>& p) {
		return sqrt(scalar(p, p));
	}
