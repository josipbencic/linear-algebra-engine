...
#include "../Core/ApplicationStateMachine.hpp"
...
State MainMenuView::operator ()() {
  ...
  switch (ans) {
    case 1: return State::TUTORIAL;
    case 2: return State::GRAM_SCHMIDT;
    case 3: return State::DISTANCE_FROM_SUBSPACE;
    case 4: return State::LINEAR_SYSTEM;
    case 0: return State::QUIT;
  }
  return State::MAIN_WINDOW;
}

math::ESpaces selectSpace() {
...
void outputGramSchmidt(vector<VectorSpace>& v) {
...
State GramSchmidtView::operator ()() {
  ...
  math::ESpaces space = selectSpace();
  ...
  switch (space) {
  case math::ESpaces::SPACE3:
    outputGramSchmidt(input.space3);
    break;
  case math::ESpaces::SPACE4:
    outputGramSchmidt(input.space4);
    break;
  case math::ESpaces::SPACE5:
    outputGramSchmidt(input.space5);
    break;
  case math::ESpaces::MATRIXSPACE3:
...
template <typename VectorSpace>
void outputDistanceFromSubspace(vector<VectorSpace>& linearHull) {
...
State LinearSystemView::operator ()() {
  ...
  string fileName; cin >> fileName;
  input.processInput(fileName.c_str(), math::ESpaces::LINEARSYSTEM);
