...
class LinearSolverLR {
  ...
  */
  ...
  /*  Permutation matrix is stored as a permutation ordering.
      It is needed since the algorithm uses partial pivoting to enable higher
     stability.
  */
  std::vector<int> P;
...
public:
  ...
  SolveUpperTriangularSystem(const mat &U, const std::vector<double> &b);

  static std::vector<double>
  SolveLowerTriangularSystem(const mat &L, const std::vector<double> &b);
  ...
  */
  static bool EqualWithRowPerm(const mat &A, const mat &B, double EPS = 1e-5,
