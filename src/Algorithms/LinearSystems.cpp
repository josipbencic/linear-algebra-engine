...
LinearSolverLR::LinearSolverLR(const mat &A) : LR(A), P(A.size()) {
...
bool LinearSolverLR::VerifyPrecompute(const mat &A) const {
  ...
  if (!LinearSystemsUtil::EqualWithRowPerm(A, X, eps, P)) {
    assert(false);
    return false;
  }
  return true;
...
Cholesky::Cholesky(const mat &A) : R(A.size(), vector<double>(A.size(), 0.0)) {
  PrecomputeR(A);
}
...
vector<double>
LinearSystemsUtil::SolveUpperTriangularSystem(const mat &U,
...
vector<double>
LinearSystemsUtil::SolveLowerTriangularSystem(const mat &L,
                                              const vector<double> &y) {
...
LinearSystemsUtil::mat LinearSystemsUtil::Multiply(const mat &L, const mat &R) {
...
LinearSystemsUtil::mat LinearSystemsUtil::MultiplyTransposed(const mat &T,
                                                             const mat &A) {
...
bool LinearSystemsUtil::EqualWithRowPerm(const mat &A, const mat &X, double EPS,
                                         vector<int> P) {
