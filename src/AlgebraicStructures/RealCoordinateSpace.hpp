...
template <std::size_t DIMENSION, typename REAL_TYPE = double> class Rn {

  friend Rn<DIMENSION, REAL_TYPE>
  operator+(const Rn<DIMENSION, REAL_TYPE> &op1,
            const Rn<DIMENSION, REAL_TYPE> &op2) {
  ...
  friend double inner_product(const Rn<DIMENSION, REAL_TYPE> &op1,
                              const Rn<DIMENSION, REAL_TYPE> &op2) {
  ...
  friend inline Rn<DIMENSION, REAL_TYPE>
  operator*(double alpha, const Rn<DIMENSION, REAL_TYPE> &op) {
  ...
  friend inline Rn<DIMENSION, REAL_TYPE>
  operator*(const Rn<DIMENSION, REAL_TYPE> &op, double alpha) {
    return alpha * op;
  }
...
public:
  ...
  Rn() { std::memset(component, 0, sizeof(component)); }
  ...
  Rn(REAL_TYPE newCoords[]) { std::copy_n(newCoords, DIMENSION, component); }
