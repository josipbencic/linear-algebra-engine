...
template <unsigned D, typename FIELD_TYPE = double> class Polynomial2 {
public:
  FIELD_TYPE coefficient[D + 1][D + 1];

  Polynomial2() { memset(coefficient, 0, sizeof(coefficient)); }
  ...
  */
  ...
  FIELD_TYPE operator()(FIELD_TYPE x, FIELD_TYPE y) const {
    ...
    for (size_t i = 0; i <= D; i++) {
      for (size_t j = 0; j <= D; j++) {
        Ret += coefficient[i][j] * PolyUtil::fastExp(x, i) *
               PolyUtil::fastExp(y, j);
      }
    }
    return Ret;
  }
